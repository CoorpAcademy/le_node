...
var Logger = (function (_Writable) {
	function Logger(opts) {
		...
		_get(Object.getPrototypeOf(Logger.prototype), 'constructor', this).call(this, { objectMode: true, highWaterMark: defaults.bufferSize });
	...
	_createDecoratedClass(Logger, [{
		...
		value: function log(lvl, _log2) {
			...
			if (_lodash2['default'].isEmpty(_log2)) {
			...
			if (_lodash2['default'].isArray(_log2)) {
				var _iteratorNormalCompletion2 = true;
				var _didIteratorError2 = false;
				var _iteratorError2 = undefined;

				try {
					for (var _iterator2 = _getIterator(_log2), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
			...
			if (_lodash2['default'].isObject(_log2)) {
				_log2 = _lodash2['default'].clone(_log2);

				if (this.timestamp) _log2[getSafeProp(_log2, 'time')] = new Date();

				if (this.withLevel && lvlName) _log2[getSafeProp(_log2, 'level')] = lvlName;

				_log2 = this[$serialize](_log2);

				if (this.console) console[getConsoleMethod(lvl)](JSON.parse(_log2));
			} else {

				_log2 = [_log2.toString()];

				if (this.withLevel && lvlName) _log2.unshift(lvlName);

				if (this.timestamp) _log2.unshift(new Date().toISOString());

				_log2 = _log2.join(' ');

				if (this.console) console[getConsoleMethod(lvl)](_log2);
			}

			if (this[$bufferFull]) {
				this.emit('error', new _error.LogentriesError(_text2['default'].bufferFull(_log2)));
				return;
			}

			this.emit('log', _log2);

			var mayWriteFurther = this.write(_log2);

			if (!mayWriteFurther) {
				this[$bufferFull] = true;
				this.once('drain', function () {
					return _this2[$bufferFull] = false;
				});
			}
		}
	}, {
				...
				while (1) switch (context$2$0.prev = context$2$0.next) {
					...
					case 2:
						...
						return new _Promise(function (resolve) {
							setTimeout(resolve, Math.pow(_this3[$failedAttempts], 1.5) * 1000);
						});
						...
						this[$connection] = new _Promise(function (resolve, reject) {
							...
							if (_this3.secure) {
								connection = _tls2['default'].connect(opts, function () {
									if (!connection.authorized) {
										_this3[$failedAttempts]++;
										var errMsg = connection.authorizationError;
										_this3.emit(new _error.LogentriesError(_text2['default'].authError(errMsg)));
									}
								});
							} else {
								connection = _net2['default'].createConnection(opts);
							}

							connection.setTimeout(_this3.timeout);

							connection.on('drain', function () {
								return setImmediate(function () {
									return _this3.emit('connection drain');
								});
							});
							...
							connection.on('error', function (err) {
								...
								if (!resolved) {
									...
									if (_this3[$failedAttempts] >= defaults.maxFailedAttempts) {
										_this3[$cannotConnect] = true;

										setTimeout(function () {
											_this3[$cannotConnect] = false;
										}, defaults.retryTimeout);
									}

									_this3[$getConnection](resolve, reject);
								}
							});
...
var BunyanStream = (function (_Writable2) {
	function BunyanStream(opts) {
		...
		_get(Object.getPrototypeOf(BunyanStream.prototype), 'constructor', this).call(this, { objectMode: true, highWaterMark: defaults.bufferSize });
